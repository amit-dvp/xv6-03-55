...
//
// proc.c file implements kernell process management
// and scheduling, owns the process table, creates processes,
// manages their states, and runs the scheduler.
// Here I added implementation of cps155() system call that used
// to present list of system processes.
...
//read current process status table
int
cps155()
{
	struct proc *p;
	...
	// note : acquire(&ptable.lock) is not blockable in the sense of 
	//        putting the calling process to sleep.  
    //        but it can spin‑wait (busy‑wait) if another CPU holds the lock.
    // 
    // acquire() in spinlock.c:
    //        calls pushcli() : disables interrupts
    //        grabs the lock
    //        keeps interrupts disabled until release()
	...
	acquire(&ptable.lock);
	    ...
	    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
	    	...
	    	if (p->pid == 1) // init process has no parent
		        ppid = 0;
	        else
		        ppid = p->parent->pid;
	...
	// sti();
      ...
      // cps155() in proc.c must return 155 because that value becomes 
      // the system‑call return value that goes back to user space.
	return 155;
}
